...
export default class ReadingResource extends Resource {
  ...
  static readings<T extends typeof Resource>(this: T) {
    const endpoint = this.detail();
    return endpoint.extend({
      url({ id, from, to }: { id: string; from?: string; to?: string }) {
        return `http://192.168.0.91:1337/room/${id}/reading?${querystring.stringify({
          from,
          to,
        })}`;
      },
      fetch({ id, from, to }: { id: string; from?: string; to?: string }) {
        return endpoint.fetch.call(this as any, { id, from, to });
      },
      schema: [
        {
